...
/**
 * The reconciliation strip at the top of the Reimbursement tab.
 *
 * Six bare figures side by side invited people to add them up by hand and get a
 * different answer, because two of the relationships between them were never on
 * screen: the families' side of the split had no tile at all, and nothing said
 * why "owed to" and "owed by" don't cancel. So the figures are laid out as the
 * two equations they actually satisfy:
 *
 *   total expenses      = split among families + covered by the unit
 *   owed to − owed by   = covered by the unit − pre-reimbursed − receipts paid back
 *
 * Both hold to the cent by construction: every group's total is allocated to
 * its shareholders by largest remainder (see engine.ts), and a person's
 * outstanding balance already nets out their prepayments and paid-back receipts
 * — which is exactly why those two terms reappear on the right of the second
 * equation instead of being subtracted twice.
 */
function Ledger({
  paysheet,
  owedToPeople,
  ...
  const total = paysheet.totalExpenses;
  const unit = round2(paysheet.totalUnitCovered ?? 0);
  const prepaid = round2(paysheet.totalPrepaid);
  const paidBack = round2(paysheet.totalReimbursed ?? 0);
  // Derived, not summed from the rows: this is the identity the equation
  // asserts, so reading it off the same source keeps the line true even for a
  // share the engine couldn't attribute to an adult.
  const families = round2(total - unit);
  const net = round2(owedToPeople - owedByPeople);
  const hosting = unit > 0.005;

  return (
    <div className="ledger">
      <div className="ledger-group">
        <div className="ledger-title">Where the money went</div>
        <div className="eq">
          <Fig
            k="Total expenses"
            v={total}
            title="Every receipt on the Expenses tab, including the travel reimbursements generated by the Travel tab."
          />
          {hosting && (
            <>
              <div className="term">
                ...
                <Fig
                  k="Split among families"
                  v={families}
                  title="The Owes (share) column below, added up — each cost group's total divided evenly among the people who attended it."
                />
              </div>
              <div className="term">
                <span className="op" aria-hidden="true">+</span>
                <Fig
                  k="Covered by the unit"
                  v={unit}
                  title="Shares belonging to attendees the troop is hosting (marked “paid by the unit” on the ⚙ tab). They still count toward each group's split, but they're billed to nobody."
                />
              </div>
            </>
          )}
        ...
        {!hosting && (
          <p className="eq-note hint">Every dollar is billed to a family — the troop isn't hosting anyone on this trip.</p>
        )}
      </div>

      <div className="ledger-group">
        <div className="ledger-title">What still has to move</div>
        <div className="eq">
          <Fig
            k="Owed to people"
            v={owedToPeople}
            tone="pos"
            title="What the troop still has to pay out, added up over everyone who is net out of pocket."
          />
          <div className="term">
            <span className="op" aria-hidden="true">−</span>
            <Fig
              ...
              v={owedByPeople}
              tone="neg"
              title="What the troop still has to collect, added up over everyone whose share came to more than they paid."
            />
          </div>
          <div className="term">
            <span className="op" aria-hidden="true">=</span>
            <Fig
              k="Net from troop funds"
              v={net}
              title="What the troop is left funding once every payment and collection above has happened."
            />
          </div>
        </div>
        {/* Why the two sides don't cancel. The identity is always the same —
            net = unit-covered − prepaid − paid-back — so it's stated the same
            way in every case; only the reason it's non-zero changes. */}
        <p className="eq-note hint">
          {hosting || prepaid + paidBack > 0.005 ? (
            <>
              {hosting
                ? `The two sides don't cancel because the troop is covering ${money(unit)} of the split itself.`
                : `The two sides don't cancel because the troop has already handed out ${money(round2(prepaid + paidBack))} ahead of settlement.`}{" "}
              Net = {money(unit)} covered by the unit − {money(prepaid)} pre-reimbursed − {money(paidBack)} in
              receipts already paid back.
            </>
          ) : (
            <>Nothing is left over: every dollar the troop pays out is a dollar it collects.</>
          )}
        </p>
      </div>
    </div>
...
/** One figure in a ledger equation. `title` is the hover explanation of what feeds it. */
function Fig({ k, v, title, tone }: { k: string; v: number; title: string; tone?: "pos" | "neg" }) {
  return (
    <div className="fig" title={title}>
      <div className="k">{k}</div>
      <div className={tone ? `v ${tone}` : "v"}>{money(v)}</div>
    </div>
  );
}
function Reimbursement({ bundle, run, busy }: TabProps) {
  ...
  return (
    ...
    <div className="card">
      <Ledger paysheet={bundle.paysheet} owedToPeople={owedToPeople} owedByPeople={owedByPeople} />
